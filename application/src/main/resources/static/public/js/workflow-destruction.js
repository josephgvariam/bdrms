(function() {
    ...
    var RecordsDestroyedView = Marionette.View.extend({
        template: '#records-destroyed-template',
        ...
        handleSaveSuccess: function(model, response)
        {
            Backbone.history.navigate(response.id + '/workflow/recordsDestroyed' , {trigger: false});
            this.options.rootView.showRequest(model);
        },
    ...
    var RootView = Marionette.View.extend({
        ...
        showRetrievalDeskVerifyRecordsView: function (request) {
            this.showChildView('main', new VerifyRecordsView({model: request, rootView: this, inventoryItemNextStatus: null, requestNextStatus: 'ASSIGNED_DESTRUCTION_DESK', title: 'Validate Records'}));
        },

        showRecordsDestroyedView: function (request) {
            this.showChildView('main', new RecordsDestroyedView({model: request, rootView: this}));
        },
    ...
    var Controller = Marionette.Object.extend({
        ...
        workflow: function(request){
            ...
            if(type === 'DESTRUCTION') {
                ...
                else if (status === 'ASSIGNED_DESTRUCTION_DESK' ){
                    rootView.showBeforeProcessRequestView(request, 'Destroy Records', 'Proceed with the destruction of the records of this request?', 'DESTROYING', 'DESTROYING', rootView.showRequest, 'startDestroyRecords');
                }
                else if (status === 'DESTROYING' ){
                    rootView.showRecordsDestroyedView(request);
                }
